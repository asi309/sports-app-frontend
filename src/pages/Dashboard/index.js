...
export default function Dashboard({ history }) {
  ...
  useEffect(() => {
    socket.on('registration_req', (data) =>
  ...
  const removeNotification = (registrationId) => {
    const newEvents = eventRequest.filter(
      (request) => registrationId !== request._id
    );
    ...
    <>
        ...
        {eventRequest.map((request) => {
          return (
            <li key={request._id}>
              <div>
                <p>
                  <strong>{request.user.email}</strong> requested to join your
                  event
      ...
      <div className="filter-panel">
        <Dropdown isOpen={dropdownOpen} toggle={toggle}>
          ...
          <DropdownMenu>
            <DropdownItem onClick={() => filterHandler(null)}>
              All Events
            </DropdownItem>
            ...
            <DropdownItem onClick={() => filterHandler('running')}>
              Running
            </DropdownItem>
